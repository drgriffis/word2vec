...
void *TrainModelThread(void *id) {
  ...
  while (1) {
    ...
    if (word == -1) continue; // seems like this will cause an infinite loop, since sentence_position never changes...
    for (c = 0; c < layer1_size; c++) neu1[c] = 0;
    for (c = 0; c < layer1_size; c++) neu1e[c] = 0;
    // calculate random sub-window size for this word (weights closer words more highly)
    next_random = next_random * (unsigned long long)25214903917 + 11;
    ...
    if (cbow) {  //train the cbow architecture
      ...
      for (a = b; a < window * 2 + 1 - b; a++) if (a != window) {
        c = sentence_position - window + a;
        if (c < 0) continue;                      // skip context slots before the sentence
        if (c >= sentence_length) continue;       // skip context slots after the sentence
        last_word = sen[c];
        if (last_word == -1) continue;
